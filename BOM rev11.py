...
# 두 번째 리스트박스 생성
list_file2 = tk.Listbox(root, selectmode=tk.EXTENDED, height=10, width=50)
list_file2.pack(side=tk.LEFT)

# 두 번째 리스트박스에 데이터 추가
for table_name in table_names:
    list_file2.insert(tk.END, table_name)
...
def remove_from_selected():
    selected_indices = list_file2.curselection()
    for index in selected_indices:
...
def update_graph():
    selected_columns = list_file.curselection()
    if not selected_columns:
        print("No selected columns.")
        return

    selected_columns = [list_file.get(idx) for idx in selected_columns]

    # 기존 코드와 동일하게 SQLite 연결 후 DataFrame을 합치는 부분
    ...
    for col in selected_columns:
        ...
        if 'Frequency' in df.columns:
            if selected_df.empty:
                ...
                # DataFrame 합치기
                selected_df = pd.merge(selected_df, df, on='Frequency', how='outer')
        else:
            print(f"Skipping table '{col}' as it doesn't have the 'Frequency' column.")

    connection.close()

    if not selected_df.empty:
        plt.clf()  # Clear the existing graph before plotting
        plot_graph(selected_df, "Graph Title", line_width=2, line_style='-')
        root.canvas.draw()
    else:
        print("Empty selected DataFrame.")

# 두 번째 리스트박스 클릭 시 실행되는 함수
def update_graph_from_list2(event):
    selected_columns = list_file2.curselection()
    if not selected_columns:
        return

    selected_columns = [list_file2.get(idx) for idx in selected_columns]
...
plot_button = tk.Button(root, text="Overlap Graphs", command=update_graph_from_list2)
...
# 두 번째 리스트박스와 이벤트를 연결
list_file2.bind("<<ListboxSelect>>", update_graph_from_list2)
