...
import sqlite3
import os
from tkinter import Tk, Button, Listbox, END, filedialog, messagebox, Toplevel, Label, Entry, ttk, Text, simpledialog, END
import re
...
def save_csv_to_database():
    ...
    def save_text(text_widget):
        ...
        if graph_info:
            cursor.execute(f"UPDATE {sanitized_table_name} SET [Text Info] = ?", (graph_info,))
            connection.commit()
            print("텍스트 정보가 저장되었습니다.")
            root.destroy()  # 입력 창을 닫음
        else:
            print("텍스트 정보가 없습니다.")

    # "텍스트 저장" 버튼을 생성합니다.
    save_button = tk.Button(root, text="텍스트 저장", command=lambda: save_text(text_widget))
    save_button.pack()
    ...
    if event.inaxes:
        ...
        if graph_type == 'Line':
            ...
            if common_prefix is not None:
                ...
                combined_text = f"SPL({spl0_y})_IMP({imp_y})    {file_name}"
                text_list.append(combined_text)
